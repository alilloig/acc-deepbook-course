...
export function registerTools(server) {
    server.tool('start', 'Start the Sui DeepBook course — returns paths, output style status, and preflight info.', {
    ...
    server.tool('runPreflightProbe', `Run a single preflight probe by id. Valid probe ids (in order): ${PROBE_ORDER.join(', ')}. Use remediate: true to trigger shell action execution (e.g. pnpm deploy-all --quick for sandbox-manifest-reachable).`, {
    ...
    server.tool('selectPath', 'Select a learning path by slug. Returns personalization prompts and initializes state.', {
        projectRoot: z.string().describe('Absolute path to the project root'),
    ...
    server.tool('setPersonalization', 'Set personalization values for the selected path. Pass empty object {} to use all defaults.', {
        projectRoot: z.string().describe('Absolute path to the project root'),
    ...
    server.tool('nextSpot', 'Get the current spot in the phase loop. Returns phase info, the substituted spot view, and ladder state.', {
        projectRoot: z.string().describe('Absolute path to the project root'),
    }, async ({ projectRoot }) => {
    ...
    server.tool('verifySpot', 'Verify the current spot. On pass, advances the cursor. On fail, leaves cursor unchanged.', {
        projectRoot: z.string().describe('Absolute path to the project root'),
    }, async ({ projectRoot }) => {
    ...
    server.tool('requestHint', 'Request a hint, reference, or auto-write for the current spot. Rung 1=hint, 2=reference, 3=auto-write.', {
        projectRoot: z.string().describe('Absolute path to the project root'),
