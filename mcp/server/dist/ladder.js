 ...
 */
export function recordRungUse(state, spotId, rung) {
 ...
 */
export function canAdvanceRung(state, spotId, rung) {
    if (rung === 1) {
        return { ok: true };
    }
 ...
 */
export async function runAutoWrite(projectRoot, spot, payload) {
    ...
    try {
        ...
        if (err instanceof PathTraversalError) {
            throw new AutoWriteError('path-traversal', `target_file path traversal blocked: ${err.message}`);
        }
        throw err;
    }
    ...
    let endLine;
    if (!rangeMatch) {
        throw new AutoWriteError('target-range-invalid', `Cannot parse target_range: '${rangeStr}' — expected format like '39-58'`);
    }
    ...
    if (startLine < 1 ||
        ...
        endLine > totalLines) {
        throw new AutoWriteError('target-range-invalid', `target_range '${rangeStr}' is out of bounds (file has ${totalLines} lines, 1-indexed)`);
    }
    ...
    try {
        ...
        if (err instanceof PathTraversalError) {
            throw new AutoWriteError('path-traversal', `spot.id path traversal blocked: ${err.message}`);
        }
        throw err;
    }
    ...
    try {
        ...
        if (e.code !== 'ENOENT') {
            throw new AutoWriteError('snapshot-write-failed', `Failed to rotate existing snapshot: ${e.message}`);
        }
    }
    ...
    try {
        ...
        const e = err;
        throw new AutoWriteError('snapshot-write-failed', `Failed to write snapshot file: ${e.message}`);
    }
    ...
    try {
        ...
        const e = err;
        throw new AutoWriteError('overwrite-failed', `Failed to overwrite target file: ${e.message}`);
    }
