...
function normalizeToArray(declared) {
 ...
 */
export function validatePersonalizationValues(values, declared) {
    ...
    for (const opt of options) {
        ...
        if (val === undefined)
            continue; // absent is valid (use defaults)
        if (opt.type === 'integer') {
            if (typeof val !== 'number' || !Number.isInteger(val)) {
                errors.push(`${opt.name} must be an integer, got ${typeof val}`);
                continue;
            }
            if (opt.range !== undefined) {
                if (val < opt.range.min) {
                    errors.push(`${opt.name} must be >= ${opt.range.min}, got ${val}`);
                }
                else if (val > opt.range.max) {
                    errors.push(`${opt.name} must be <= ${opt.range.max}, got ${val}`);
                }
            }
        }
            ...
            if (opt.enum !== undefined && !opt.enum.includes(val)) {
                errors.push(`${opt.name} must be one of [${opt.enum.join(', ')}], got '${val}'`);
            }
        }
    }
    if (errors.length > 0) {
 ...
 */
export function substitutePromptOnly(prompt, values) {
    return prompt.replace(/\{\{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}/g, (_match, key) => {
